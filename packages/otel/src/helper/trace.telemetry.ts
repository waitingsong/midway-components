...
import { InitTraceOptions, InstrumentationList, PropagatorList, SpanExporterList } from '../lib/types'

import { regTraceInstrumDns } from './dns.instrum'
import { regTraceInstrumHttp } from './http.instrum'
import { regTraceInstrumKnex } from './knex.instrum'
import { regTraceInstrumPg } from './pg.instrum'
...
export function initTrace(options: InitTraceOptions): InitTraceReturnType {
  ...
  regTraceInstrums(otelConfig, provider)
 ...
 */
function regTraceInstrums(
  config: InitTraceOptions['otelConfig'],
  tracerProvider: NodeTracerProvider,
): void {

  const instrumListOptionsMap = new Map<InstrumentationList, InstrumentationOption>()

  config.instrumentations?.forEach((instrum) => {
    if (instrumListOptionsMap.has(instrum)) {
      return
    }
    switch (instrum) {
      case InstrumentationList.dns: {
        const opts = regTraceInstrumDns()
        instrumListOptionsMap.set(instrum, opts)
        break
      }

      case InstrumentationList.http: {
        const opts = regTraceInstrumHttp()
        instrumListOptionsMap.set(instrum, opts)
        break
      }

      case InstrumentationList.koa: {
        const opts = new KoaInstrumentation()
        instrumListOptionsMap.set(instrum, opts)
        break
      }

      case InstrumentationList.knex: {
        const opts = regTraceInstrumKnex()
        instrumListOptionsMap.set(instrum, opts)
        break
      }

      case InstrumentationList.pg: {
        const opts = regTraceInstrumPg()
        instrumListOptionsMap.set(instrum, opts)
        break
      }

      default: {
        throw new Error(`Instrumentation ${instrum} not implemented. Use registerInstrumentations() manually.`)
      }
    }
  })

  const instrumentations = Array.from(instrumListOptionsMap.values())
