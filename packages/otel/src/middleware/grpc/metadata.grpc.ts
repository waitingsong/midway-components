...
export interface Metadata {
  ...
  set<Key extends string>(
    key: Key,
    value: MetadataValue<Key> | MetadataValue<Key>[],
  ): Metadata
  /**
   ...
   * either, because some gRPC implementations may not support it.
   */
  getAll<Key extends string>(key: Key): MetadataValue<Key>[]
  /**
...
export type MetadataInit =
  | Metadata
  | Iterable<[string, string | Uint8Array | (string | Uint8Array)[]]>
  | Record<string, string | Uint8Array | (string | Uint8Array)[]>
...
export const Metadata = function Metadata(init?: MetadataInit): Metadata {
  ...
  const metadata = {
    set(key: string, value: string | Uint8Array | (string | Uint8Array)[]) {
    ...
    getAll<Key extends string>(key: string): MetadataValue<Key>[] {
      key = normalizeKey(key)

      return (data.get(key) ?? []) as MetadataValue<Key>[]
    },
