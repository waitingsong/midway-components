...
describe(fileShortPath(import.meta.url), function () {
  ...
  const path = `${apiBase.trace_log}/${apiMethod.after_throw}`
  ...
  it(`Should ${path} work`, async () => {
    ...
    const opt1: AssertsOptions = {
      ...
      logs: [
        ...
        { event: 'afterThrow', args0: foo, instanceName, methodName: methodName1 },
        { event: 'exception', 'exception.message': 'foo-error' },

        { event: 'before', args0: bar, instanceName, methodName: methodName2 },
        { event: 'afterThrow', args0: bar, instanceName, methodName: methodName2 },
